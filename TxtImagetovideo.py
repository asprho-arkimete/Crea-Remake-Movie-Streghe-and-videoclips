...
def crea_elementi(root, frame, canvas, scrollbar, scrollable_frame, photos, text):
    global check_var,checkboximagevideo,photosclips,listvary,listlong,longclip,steps,cfg,list_seed,button,voiceelevellabs,list_dir,list_sound,list_combovoices,list_positives,list_negatives,list_combomodelaudios,Loading,pathfacebox,combostreghe
    ...
    def generaimmagine(desc_englist,k,negativo,c,s):
        global w,h,combobox,comboboxresolution,list_seed,check_seed,checkboxseed,pathfacebox,combostreghe
            ...
            with torch.cuda.amp.autocast():
                ...
                if combostreghe.get()!="Nessun Personaggio":
                    if pathfacebox != "nessuna foto" and pathfacebox is not None and pathfacebox != "":
                        ...
                        print(f" volto selezionato: {selezionavolto}")
                    else:
                        # seleziona volto
                        print(f"path face: {pathface}")
                        if pathface!="nessunviso" and pathface!= None and pathface!="":
...
""")
    ...
    def make_regenerate_function(i, entry, entryneg,cfg,steps):
        ...
        def regenerate_image():
            ...
            list_positives[i] = entry.get('1.0', 'end').strip()
            list_negatives[i] = entryneg.get('1.0', 'end').strip()
        ...
        def regenerate_video():
            ...
            if os.path.exists(f'.\\PIA\\example\\result\\{i}\\{v}_sim_3.gif'):
                ...
                # Aggiorna i dati dell'entry nella lista
                if list_positives[i] != entry.get('1.0', 'end').strip():
                    list_positives[i] = entry.get('1.0', 'end').strip()
                if list_negatives[i] != entryneg.get('1.0', 'end').strip():
                    list_negatives[i] = entryneg.get('1.0', 'end').strip()
...
def main():
    ...
    def saving():
        ...
        with open(temp_filename, 'w') as fsave:
            ...
            for i, line in enumerate([
                ...
                f"STEPS:{steps.get()}",
                f"CFG:{cfg.get()}",
                f"PROMPTS_POSITIVE: {list_positives}",
                f"PROMPTS_NEGATIVE: {list_negatives}",
    ...
    # Crea picturebox
    picturebox = Canvas(model_frame, width=128, height=128, background="pink")
    ...
    uploadfoto = ttk.Button(model_frame, text="Upload Faccia")
    uploadfoto.pack(side="top")
